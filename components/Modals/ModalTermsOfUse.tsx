...
const ModalTermsOfUse = ({ show }: any) => {
  return ( 
    <Modal__main style = {{ zIndex: show ? 100 : -1, display: show ? 'flex' : 'none' }}>
      ...
      <p>В данный момент активно ведется разработка и бот находится в бета-тестировании, далее цена может измениться.</p>
      <p>Во время бета-тестирования цена является фиксированной. В дальнейшем будет введена ежемесячная подписка.</p>
