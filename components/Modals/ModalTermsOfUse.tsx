...
const ModalTermsOfUse = ({ show }: any) => {
  return ( 
    <Modal__main style = {{ zIndex: show ? 100 : -1, display: show ? 'flex' : 'none' }}>
      ...
      <p>Если бот перестает работать или не получается оплатить подписку Вы можете обратиться по контактам указанным ниже.</p>
      <br/>
      <p><strong>Условия возврата</strong></p>
      <p>Возврат денежных средств (без учета комиссии) возможен в течение 3х дней, после покупки, при условии, что ранее бот не приобретался для данного канала.</p>
      <br/>
