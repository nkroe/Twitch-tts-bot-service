...
export const followChannel = (c: number, id: any) => {
  Settings.findOne({ secret: SESSION_SECRET }).then((settings: DBSettings | null) => {
    if (settings) {
      if (c <= 1) {
        axios
          .put(`https://api.twitch.tv/kraken/users/464971806/follows/channels/${id}`, '', {
            headers: {
          ...
          .catch((e: { response: { data: any } }) => {
            ...
            axios
              .post(
                `https://id.twitch.tv/oauth2/token?grant_type=refresh_token&refresh_token=${settings.refreshToken}&client_id=${FOLLOW_ID}&client_secret=${FOLLOW_SECRET}`
              )
              .then((_refresh: { data: { access_token: any; refresh_token: any } }) => {
                Settings.updateOne(
                  {
