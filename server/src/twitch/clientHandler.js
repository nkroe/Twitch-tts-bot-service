        ...
        getChannelInfo(target.slice(1)).then(channelInfo => {
            ...
            if (/^!fake /gi.test(msg) && (type !== '4')) {
                if (isPrem()) {
                    ...
                    if (isSub()) {
                        updateUsers(user, target, context)
                    } else
            ...
            if (isPrem()) {
                if (/^!fakesub$/gi.test(msg)) {
                    updateType('2', 'сабы и выше', target, context);
                } else

                if (/^!fakevip$/gi.test(msg)) {
                    updateType('3', 'випы и выше', target, context);
                } else

                if (/^!fakemsg$/gi.test(msg)) {
                    updateType('4', 'выделенные сообщения', target, context);
                } else

                if (/^!fakeall$/gi.test(msg)) {
                    updateType('1', 'всех', target, context);
                } else

                if (/^!fakeprem$/gi.test(msg)) {
                    updateType('5', 'премиум пользователи', target, context);
                } else
