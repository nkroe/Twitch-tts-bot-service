...
export const getChannelInfo = (chan: any) =>
  new Promise(res => {
    event.emit('getInfo', chan);
    event.on(`getInfoRes-${chan}`, (data: unknown) => res(data));
  });

export const emitPlay = (t: any, target: { slice: (arg0: number) => void }) =>
  event.emit('play', {
...
export const updateType = (
  ...
  message: string,
  client: any,
  target: { slice: { (arg0: number): void; (arg0: number): void } },
...
export const updateUsers = (
  u: any,
  text: string,
  target: { slice: (arg0: number) => void },
  context: { username: any }
) => {
  const _type = u ? 2 : 1;
